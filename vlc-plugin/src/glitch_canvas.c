...
/* MSVC does not provide the POSIX ssize_t type; supply it before VLC headers
   pull in vlc_arrays.h and vlc_configuration.h which use it. */
#ifdef _MSC_VER
# include <BaseTsd.h>
...
/* vlc_common.h forward-declares filter_sys_t as an opaque struct:
 *   typedef struct filter_sys_t filter_sys_t;
 * We must complete that declaration rather than redefine it via a typedef to
 * an anonymous struct (MSVC treats the two as different basic types). */
struct filter_sys_t {
